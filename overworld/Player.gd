...
func move(delta):
	...
	if door_ray.is_colliding():
		...
		if percent_moved >= 2.0:
			...
			var dust_effect = landing_effect.instance()
			dust_effect.position = position
			get_tree().current_scene.add_child(dust_effect)
		else:
			shadow.visible = true
			jumping_over_ledge = true
			var input = input_direction.y * TILE_SIZE * percent_moved
			position.y = initial_position.y + (-0.24 - 0.1325 * input + 0.0125 * pow(input, 2))
